...
const Tutorial = ({ history, location }) => {
  ...
  const userType = useMemo(() => typelist.find((e) => e.type === query.type), [
    query.type,
  ]);
  ...
  return (
    <>
      <div
        ...
        style={{
          transform: screenUp ? `translateY(-${upHeight}px)` : "translateY(0)",
        }}
      >
        <ActionArea
      ...
      <div className={`alert ${transitionToggle.alert ? "alerttoggle" : ""}`}>
        {transitionToggle.background ? (
          ...
          <>
            <div
              className={`congraturation ${
                transitionToggle.congraturation ? "congraturationToggle" : ""
              }`}
            >
              <span className="title"> {value.title} 완료</span>
              <span className="body">
                <AddEnter array={value.success} />
              </span>
              <button
                type="button"
                onClick={nextButtonClick}
                className={
                  transitionToggle.congraturationButton ? "toggle" : ""
                }
                ref={nextButton}
              >
                다음
        ...
        >
          <CircularProgressbar
            ...
            styles={buildStyles({
              ...
              // Colors
              pathColor: `rgba(0, 115, 240, ${percent})`,
              textColor: "#fff",
              trailColor: "#d6d6d6",
